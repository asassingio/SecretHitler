...
public sealed class GameViewShowCardMenu : Menu
{
	...
	{
		MyPlayerController playerControllerThatWillSee = GameManager.Instance.GetPlayerByName(playerThatWillSeeDropdown.captionText.text);
		MyPlayerController playerControllerToSee = GameManager.Instance.GetPlayerByName(playerToSeeDropdown.captionText.text);
		string playerToSeeName = playerControllerToSee.GetPlayerName();

		SecretRoleEnum cardToShow;
		switch (playerControllerToSee.GetSecretRole())
		{
			...
			case SecretRoleEnum.Banchiere:
				...
				break;
			default:
				cardToShow = playerControllerToSee.GetSecretRole();
				break;
		}
